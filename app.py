...
st.markdown("""
    ...
    /* Yanıp sönen kayıt ışığı animasyonu */
    @keyframes blink { 50% { opacity: 0; } }
    .recording-dot {
        height: 15px; width: 15px; background-color: red; border-radius: 50%;
        display: inline-block; animation: blink 1s linear infinite;
    }
    </style>
...
def show_chat():
    with st.sidebar:
        ...
        # Varsayılanı İngilizce yaptık (Index=0)
...
# --- MOD 2: KONFERANS (DÜZELTİLEN KISIM) ---
def show_conf():
    with st.sidebar:
        if st.button("⬅️ Menüye Dön"): st.session_state.page = "home"; st.rerun()
        st.header("🎙️ Konferans Ayarları")
        # Varsayılan hedef dil English yapıldı (Index 1)
    ...
    st.markdown(f"### 🎙️ Konferans Modu (Çıktı: {target_lang})")
    st.info("Bu modda ortamı dinlerim ve seçtiğiniz dile çeviririm.")
    
    # 300 saniye (5 dakika) bekleme süresi
    audio = audio_recorder(text="Dinlemeyi Başlat / Bitir", icon_size="5x", recording_color="red", pause_threshold=300.0)
    
    if audio:
        # Kayıt bitince buraya girer
